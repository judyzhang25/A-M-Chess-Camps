class InstructorsController < ApplicationController
  ...
  def index
    @instructors = Instructor.all.alphabetical.paginate(:page => params[:instructors]).per_page(12)
  end
  ...
  private
    ...
    def instructor_params
      params.require(:instructor).permit(:username, :password, :password_confirmation, :first_name, :last_name, :bio, :email, :phone, :photo, :active)
    end
    
    def user_params
      params.require(:instructor).permit(:username, :password, :password_confirmation, :phone, :email)
    end
end